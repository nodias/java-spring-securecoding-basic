...
import org.aspectj.lang.annotation.Before;
	...
	public class AnnotLoggingAspect {
	 ...
	 @Around("execution(public * kr.co.openeg.aop.test.*.println*(..))")
...
/* https://www.egovframe.go.kr/wiki/doku.php?id=egovframework:rte:fdl:aop:aspectj
	...
	** 충고(Advice) 정의하기
	...
	충고(Advice)는 관점(Aspect)의 실제 구현체로 포인트컷 표현식과 일치하는 결합점에 삽입되어 동작할 수 있는 코드이다.
	충고는 결합점과 결합하여 동작하는 시점에 따라 before advice, after advice, around advice 타입으로 구분된다.
	...
	** 예외 발생의 경우
	...
	testAnnotationAspectWithException() 함수는 대상 메소드에 오류가 발생한 사례를 보여준다. 
	egovframework.rte.fdl.aop.sample 패키지에 속하는 AnnotationAdviceSample 클래스의 someMethod() 메소드는 before, after throwing, after finally, around 충고(Advice)가 적용된다.
